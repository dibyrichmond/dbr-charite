    ...
    for (const session of sessions) {
      ...
      for (const admin of admins || []) {
        await fetch(`${appUrl}/api/email`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ to: admin.email, subject: `Bilan disponible — ${participant.name}`, html: `<!DOCTYPE html><html><body style="background:#0D0D0D;font-family:system-ui;padding:40px 24px;"><div style="max-width:560px;margin:0 auto;background:#1A1A1A;border:1px solid rgba(74,184,232,0.3);border-radius:16px;padding:32px;"><div style="font-size:10px;letter-spacing:3px;color:#4AB8E8;margin-bottom:12px;">NOTIFICATION ADMIN</div><div style="font-size:16px;font-weight:700;color:#F0F0F0;margin-bottom:12px;">Bilan disponible — ${participant.name}</div><a href="${appUrl}" style="display:inline-block;margin-top:16px;padding:10px 20px;background:rgba(74,184,232,0.15);border:1px solid rgba(74,184,232,0.4);border-radius:8px;color:#4AB8E8;font-size:13px;text-decoration:none;">Ouvrir la console →</a></div></body></html>` }),
        })
      }
    }
