...
export default function Auth({ onLogin }) {
  ...
  const pwdField = (ph, val, set, show, setShow, autoComplete = "current-password") => (
    <div style={{ position: "relative", marginBottom: 12 }}>
      <input value={val} onChange={e => set(e.target.value)} type={show ? "text" : "password"} placeholder={ph} autoComplete={autoComplete}
  ...
  const inp = (ph, val, set, type = "text", autoComplete = "off") => (
    <input value={val} onChange={e => set(e.target.value)} type={type} placeholder={ph} autoComplete={autoComplete}
  ...
  const handleEnter = (e) => {
    ...
    else if (mode === "reset-confirm") doResetConfirm();
  };
    ...
    <div style={{ minHeight: "100vh", background: T.gradient, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ maxWidth: 420, width: "100%" }}>
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          ...
          <div style={{ fontSize: 11, color: T.muted, marginTop: 6 }}>Compagnon {APP_NAME} · Vivre pleinement. Avancer chaque jour.</div>
        </div>
        <div style={{ background: T.cardBg, border: `1px solid rgba(232,84,10,0.3)`, borderRadius: 16, padding: "32px 24px" }} onKeyDown={handleEnter}>
          ...
          {mode === "reset-confirm" && <>
            ...
            {pwdField("Nouveau mot de passe", newPwd, setNewPwd, showNewPwd, setShowNewPwd, "new-password")}
            {pwdField("Confirmer le nouveau mot de passe", newPwd2, setNewPwd2, showNewPwd2, setShowNewPwd2, "new-password")}
          </>}
          {mode !== "reset-confirm" && mode === "register" && inp("Prénom", name, setName, "text", "given-name")}
          {mode !== "reset-confirm" && inp("Email", email, setEmail, "email", "email")}
          {mode !== "reset" && mode !== "reset-confirm" && pwdField("Mot de passe", pwd, setPwd, showPwd, setShowPwd, mode === "login" ? "current-password" : "new-password")}
          {mode === "register" && pwdField("Confirmer le mot de passe", pwd2, setPwd2, showPwd2, setShowPwd2, "new-password")}
          {mode === "register" && inp("Code d'invitation", invCode, setInvCode, "text", "off")}
        ...
        <div style={{ textAlign: "center", marginTop: 16, fontSize: 11, color: T.muted, opacity: 0.5 }}>Humilité · Sincérité · Bienveillance</div>
      </div>
    </div>
  );
}
